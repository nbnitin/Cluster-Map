...
@property (nonatomic,assign) CLLocationCoordinate2D   annotationLocation;
...
- (instancetype)initWithAnnotation:(id<CalloutAnnotationProtocol>)annotation reuseIdentifier:(NSString *)reuseIdentifier {
    ...
    if (self)
    {
        _annotationLocation = [annotation coordinate];
        _yShadowOffset = 3;
...
{
    CGRect contentRect = CGRectOffset(self.bounds, 10, 4);
    contentRect.size = [self contentSize];
    self.contentView.frame = contentRect;
    self.viewController.view.frame = CGRectMake(4,4, _calloutSize.width, _calloutSize.height);
    [self.viewController.view setNeedsDisplay];
}

- (CGSize)contentSize
{
    return CGSizeMake(_calloutSize.width + 8, _calloutSize.height + 8);
}
...
- (void)adjustMapRegionIfNeeded
{
    ...
    //Longitude
    self.xPixelShift = 0;
    if ([self relativeParentXPosition] < 0)
    {
        self.xPixelShift = 10 - [self relativeParentXPosition];
    }
    else if ([self relativeParentXPosition] > self.mapView.frame.size.width - 90)
    {
        self.xPixelShift = (self.mapView.frame.size.width - 70) - [self relativeParentXPosition];
    }
    ...
    if (pixelsFromTopOfMapView < -50)
    {
        yPixelShift = 7 - (pixelsFromTopOfMapView + 65);
    }
    else if (pixelsFromBottomOfMapView < 100)
    {
        yPixelShift = -(100 - pixelsFromBottomOfMapView);
    }
    ...
    self.centerOffset = CGPointMake(self.centerOffset.x - self.xPixelShift, self.centerOffset.y + 15);
...
- (void)drawRect:(CGRect)rect
{
    ...
    color = [[UIColor blackColor] colorWithAlphaComponent:0.6];
    [color setFill];
    CGContextAddPath(context, path);
    CGContextSaveGState(context);
    CGContextSetShadowWithColor(context, CGSizeMake (0, self.yShadowOffset), 6, [UIColor colorWithWhite:0 alpha:.7].CGColor);
    ...
    // Nasib :this value for gradiant drawing if you are dont want it then don't devide height by 2
